...
class Planner:
    def __init__(self):
        ...
        self.ground_truth_pos_var  = None
        self.ground_truth_ang_var = None
        self.ground_sigma_p = None
        self.ground_truth_sigma_p_inv = None
        self.grond_truth_vel = None
        self.ground_truth_pos = None
        self.ground_truth_ang = None

        self.ground_truth_resolution = None
        self.ground_truth_height = None
        self.ground_truth_width = None

        self.ground_truth_offset_x = None
        self.ground_truth_offset_y = None
        self.ground_truth_grid_map = None
        ...
        self.classes = ['towel', 'objects', 'lighting', 'stool', 'counter',
                        ...
                        'banana', 'cheezit']
        self.target_class = ['towel', 'chair', 'tv_monitor']
        ...
        if self.ground_truth:
            self.odom_sub = rospy.Subscriber("/ground_truth/odom", Odometry,
                                         self.odom_callback) # /rtabmap/odom
            self.semantic_map_sub = rospy.Subscriber("/ground_truth/semantic_map",
                                                     Object2DArray,
                                                     self.semantic_map_callback)  # /semantic_map
            self.map_sub = rospy.Subscriber("/ground_truth/map", OccupancyGrid,
                                            self.map_callback)  # /rtabmap/grid_map
        else:
            ...
            self.map_sub = rospy.Subscriber("/rtabmap/grid_map", OccupancyGrid,
                                            self.map_callback)  # /rtabmap/grid_map
        # self.ground_truth_odom_sub = rospy.Subscriber("/ground_truth/odom", Odometry,
        #                                  self.ground_truth_odom_callback)


        # self.ground_truth_semantic_map_sub = rospy.Subscriber("/ground_truth/semantic_map",
        #                                          Object2DArray,
        #                                          self.ground_truth_semantic_map_callback)


        #self.ground_truth_map_sub = rospy.Subscriber("/ground_truth/map", OccupancyGrid,
                                        #self.ground_truth_map_callback)

        self.ground_truth_debug_pub = rospy.Publisher("/ground_truth/debug/map_image", Image, queue_size=10)
        ...
        self.c2 = rospy.ServiceProxy('calculate_distance', CalculateDistance)
    ...
    def ground_truth_odom_callback(self, msg):
        self.ground_truth_pos_var = msg.pose.covariance[0]
        self.ground_truth_ang_var = msg.pose.covariance[-1]

        self.ground_sigma_p = np.diag([self.ground_truth_pos_var, self.ground_truth_pos_var, self.ground_truth_ang_var])
        if self.ground_truth_pos_var > 0 and self.ground_truth_ang_var > 0:
            self.ground_truth_sigma_p_inv = inv(self.ground_sigma_p)
        else:
            self.ground_truth_sigma_p_inv = np.diag([np.inf, np.inf, np.inf])

        self.grond_truth_vel = msg.twist.twist.linear.x
        self.ground_truth_pos = np.asarray([msg.pose.pose.position.x,
                                   msg.pose.pose.position.y])

        self.ground_truth_ang = -tf.transformations.euler_from_quaternion(
                [msg.pose.pose.orientation.x,
                 msg.pose.pose.orientation.y,
                 msg.pose.pose.orientation.z,
                 msg.pose.pose.orientation.w])[1]
        print(self.ground_truth_ang)

    def construct_dfa(self):
        self.dfa = DFA(0, ['1', '2', '3', 'D'], final_states={1, 4}, goal_states={3:0.1, 2:0.9, 1:1})
        # '1' Complete task A
        ...
        self.dfa.add_transition('4', 2, 2)
        self.dfa.add_transition('4', 3, 3)

        self.dfa.add_transition('1', 0, 1)
        self.dfa.add_transition('2', 0, 2)
        self.dfa.add_transition('3', 0, 3)

        self.dfa.add_transition('1', 3, 1)
        self.dfa.add_transition('2', 3, 2)
        self.dfa.add_transition('3', 3, 3)

        self.dfa.add_transition('1', 2, 1)
        self.dfa.add_transition('2', 2, 2)
        self.dfa.add_transition('3', 2, 3)
    ...
    def trial(self, s0):
        ...
        while not done:
            ...
            state_grid = np.unravel_index(state[0], (self.old_height, self.old_width))

            debug_image_color[state_grid[0], state_grid[1], :] = [189, 114, 0]

            # if self.old_obstacle_prob[state_grid[0], state_grid[1]] > 0:
            #     print('trial hit obstacles')
    ...
    def wild_fire(self, Vh, non_states, pos_grid):
        if np.amax(Vh) > 0:
            ...
            while Vh[pos_grid[1], pos_grid[0]] == 0:
                ...
                Vh[non_states] = 0
                count += 1
            Vh[Vh == 0] = np.amax(Vh) * np.power(self.gamma,
                                                 count)
        return Vh
        ...
        while not rospy.is_shutdown():
            if self.obstacle_map is not None and not self.initial_obs and not self.mission_status:
                ...
                if self.replan:
                    ...
                    req = self.c2(True)
                    print('distance', req.d, 'dfa state', self.dfa_state)
                    self.iteration = 0
                    self.mutex.acquire()
                    print('acquire mutex')
                    ...
                    if self.dfa_state in self.dfa.goal_states:
                        self.dfa.goal_states[self.dfa_state] = 0
                    ...
                    Vh = self.old_frontier_prob*self.dfa.goal_states[3]
                    Vh = self.wild_fire(Vh, non_states, pos_grid)

                    Vh_A = np.zeros_like(Vh)
                    Vh_B = np.zeros_like(Vh)
                    ...
                    if A:
                        ...
                        Vh_A = self.dfa.goal_states[1]*self.old_goal_prob[obj_id]
                        Vh_A = self.wild_fire(Vh_A, non_states, pos_grid)
                    ...
                    if B:
                        obj_id = self.old_obj_of_interest[target_class_index]
                        Vh_B = self.dfa.goal_states[2] * self.old_goal_prob[
                            obj_id]
                        print('possible ', target_class, ' identified')
                    ...
                    if C:
                        ...
                        Vh_B += self.dfa.goal_states[2] * self.old_goal_prob[
                            obj_id]
                        Vh_B = self.wild_fire(Vh_B, non_states, pos_grid)
                        print('possible ', target_class, ' identified')
                    ...
                    for j in range(states.shape[0]):
                        ...
                        state = states[j]
                        self.V[(state_index, 0)] = Vh[state[0], state[1]] + Vh_A[state[0], state[1]] + Vh_B[state[0], state[1]]
                        self.V[(state_index, 2)] = Vh[state[0], state[1]] + Vh_A[state[0], state[1]]
                        self.V[(state_index, 3)] = Vh_A[state[0], state[1]] + Vh_B[state[0], state[1]]
                ...
                self.goal_status = self.dfa_state in self.dfa.final_states or (self.dfa_state in self.dfa.goal_states and self.dfa.goal_states[self.dfa_state] > 0)
    ...
    def odom_callback(self, msg):
        ...
        if self.pos_var > 0 and self.ang_var > 0:
            self.sigma_p_inv = inv(self.sigma_p)
        else:
            self.sigma_p_inv = np.diag([np.inf, np.inf, np.inf])
        ...
        if self.ground_truth:
            ...
            self.ang = 2*np.arctan2(np.sqrt(msg.pose.pose.orientation.x**2+
                                            msg.pose.pose.orientation.y**2+
                                            msg.pose.pose.orientation.z**2),
                                    msg.pose.pose.orientation.w)

            if msg.pose.pose.orientation.y* np.sin(self.ang/2) > 0:
                self.ang = -self.ang
            self.ang -= np.pi/2
        else:
        ...
        if self.initial_obs and self.old_ang is not None:
            ...
            if (self.observed_ang > 2 * np.pi and not self.ground_truth) \
                or (self.observed_ang < -2*np.pi and self.ground_truth):
    ...
    def to_grid_ground_truth(self, pose):
        return (int(round((pose[0] - self.ground_truth_offset_x) / self.ground_truth_resolution)),
                self.ground_truth_height - int(
                    round((pose[1] - self.ground_truth_offset_y) / self.ground_truth_resolution)))
    ...
    def ground_truth_map_callback(self, msg):
        """Callback from map
        :type msg: OccupancyGrid
        """
        np_data = np.array([color_converter(e) for e in msg.data])

        self.ground_truth_resolution = msg.info.resolution
        self.ground_truth_height = msg.info.height
        self.ground_truth_width = msg.info.width

        self.ground_truth_offset_x = msg.info.origin.position.x
        self.ground_truth_offset_y = msg.info.origin.position.y

        self.ground_truth_grid_map = np.flipud(np.reshape(np_data,
                                                  (self.ground_truth_height, self.ground_truth_width)
                                                  )).astype('uint8')

        if self.ground_truth_pos is not None:
            cv2.circle(self.ground_truth_grid_map, self.to_grid_ground_truth(self.ground_truth_pos),
                           5, 0, -1)

        for obj_id, obj in self.ground_truth_objects.items():
            obj_pose = obj.pos
            center_coordinates = self.to_grid_ground_truth(obj_pose)
            print(center_coordinates)

            cv2.circle(self.ground_truth_grid_map, center_coordinates, 50, 120, -1)

        image_message= self.bridge.cv2_to_imgmsg(self.ground_truth_grid_map.astype(np.uint8), encoding="mono8")
        self.ground_truth_debug_pub.publish(image_message)
    ...
    def map_callback(self, msg):
        ...
        if self.mutex.acquire(False):
            ...
            if self.pos is not None:
                ...
                self.findFrontier()
                if self.ground_truth:
                    self.frontier_map = np.zeros_like(self.grid_map_grey)
                    self.frontier_phase = np.zeros_like(self.grid_map_grey)
                ...
                if self.ground_truth:
                    self.obstacle_prob = self.obstacle_map
                    self.frontier_prob = self.frontier_map

                    for obj_id in self.sightings_map:
                        self.sightings_prob[obj_id] =self.sightings_map[obj_id]

                    for obj_id in self.goal_map:
                        self.goal_prob[obj_id] = self.goal_map[obj_id]
                else:
        ...
        for target_class_index in self.obj_of_interest:
            ...
            for angle in angles:
                ...
                for pt in discrete_line:
                    ...
                    if norm(start - np.asarray(pt), ord=1) < th:
                        continue
    ...
    def semantic_map_callback(self, msg):
        ...
        p2 = self.pos + 10 * self.resolution * np.asarray(
            [np.cos(self.ang),
             np.sin(self.ang)])
        p2 = self.to_grid(p2)
        cv2.arrowedLine(grid_map, pos_grid, p2, color=(32, 177, 237))
    ...
    def ground_truth_semantic_map_callback(self, msg):
        for obj_msg in msg.objects:
            obj_id = obj_msg.id
            covariance = np.reshape(obj_msg.covariance, (2, 2))
            obj_pose = np.asarray([obj_msg.x, obj_msg.y])
            ...
            if obj_id not in self.ground_truth_objects:
                self.ground_truth_objects[obj_id] = MapObject(obj_id, obj_pose, covariance,
                                                 class_probs)
            else:
                obj = self.ground_truth_objects[obj_id]
                obj.update(obj_pose, covariance, class_probs)
