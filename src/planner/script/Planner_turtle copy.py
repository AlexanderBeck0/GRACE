...
class Planner:
    ...
    def run(self):
        while not rospy.is_shutdown():
            ...
            if self.initial_obs:
                if goal_state == GoalStatus.SUCCEEDED or goal_state == GoalStatus.LOST or goal_state == GoalStatus.ABORTED:
                    goal = MoveBaseGoal()
                    goal.target_pose.header.frame_id = 'base_link'
                    goal.target_pose.header.stamp = rospy.Time.now()
                
                    if not self.observed_ang:
                        goal.target_pose.pose.position.x = 4 #1.5 meters
                        goal.target_pose.pose.position.y = 0
                        goal.target_pose.pose.orientation.w = 1
                    else: 
                        q = tf.transformations.quaternion_about_axis(2*np.pi / 3, (0, 0, 1))
                    ...
                    if goal_state == GoalStatus.SUCCEEDED or goal_state == GoalStatus.LOST:
                        self.observed_ang += 1
                        rospy.loginfo("Hooray, the base reached one goal")
                    
                    if self.observed_ang >=  4:
                ...
                if self.obstacle_map is not None and not self.mission_status:
                    ...
                    if self.goal_status and goal_state == GoalStatus.SUCCEEDED:
                        ...
                        if not self.ang_goal1_status:
                            ...
                            q = tf.transformations.quaternion_about_axis(angle_error, (0, 0, 1))
                            goal.target_pose.pose.orientation.x = q[0]  
                            goal.target_pose.pose.orientation.y = q[1]  
                            goal.target_pose.pose.orientation.z = q[2]  
                            goal.target_pose.pose.orientation.w = q[3]  
                        ...
                        if self.iteration > 300 and (goal_state == GoalStatus.SUCCEEDED or self.goal_aborted):
