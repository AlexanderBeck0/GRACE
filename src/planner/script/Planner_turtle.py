...
class Planner:
    def __init__(self):
        ...
        self.ang_command = None

        self.ang_error_array = np.zeros(100)
        self.vel_error_array = np.zeros(100)
        ...
        self.planner_type = "initialization"
        ...
        self.target_class = 'microwave'

        self.obj_of_interest = None
        self.max_class_prob = 0

        self.agent_radius = 0.03 # 0.177 
        ...
        self.replanning_sub = rospy.Subscriber("/replanning_signal", std_msgs.msg.Bool, self.replanning_callback)
        #rostopic pub /replanning_signal std_msgs/Bool 1
        rospy.on_shutdown(self.save_data)

    def save_data(self):
        print('planning time is ', self.planning_time)
        print('total time is ', self.total_time)
        print('total path length ',self.path_length)
        print('num planning cycle is ', self.num_cycle)
        with open(os.path.join(os.path.dirname(__file__), "../../../out/error_log.pkl"), "wb") as fp:  # Pickling
            pickle.dump([self.vel_error_log, self.ang_error_log], fp)

    def replanning_callback(self, msg):
        if msg.data and not self.replan:
            self.replan = True

    def img_callback(self, msg):
        file = open(os.path.join(os.path.dirname(__file__), "../../../out/MissionStatus/MissionStatus.txt"), "a")
        ...
        while not rospy.is_shutdown():
            ...
            if self.initial_trans:
                if goal_state == GoalStatus.LOST or goal_state == GoalStatus.ABORTED:
                    ...
                    goal.target_pose.pose.position.x = 1.2 #1.7 meters
                ...
                if self.obstacle_map is not None and not self.mission_status:
                    ...
                    if self.replan:
                        ...
                        if self.grid_map_grey is not None:
                            cv2.imwrite(os.path.join(os.path.dirname(__file__), '../../../out/Reward/map_') +  str(img_id)+'.jpg', self.grid_map_grey)
                        ...
                        if self.max_class_prob > 0.6:
                            for obj_id in self.sightings_map:
                                if self.sightings_map[obj_id] is not None:
                                    ...
                                    cv2.imwrite(os.path.join(os.path.dirname(__file__), '../../../out/Reward/goal_') +  str(img_id)+'.jpg', temp.astype(np.uint8))
                    ...
                    if self.goal_status:
                        ...
                        if not self.ang_goal1_status:
                            ...
                            if np.abs(angle_error) < self.angular_error_threshold:
                                ...
                                move_cmd.angular.z = self.angle_tracking(angle_error)
    ...
    def angle_tracking(self, ang_error):
        ...
        if np.abs(ang_error) > (
            ...
            angular_correction = np.abs(ang_error) / 2.0
    ...
    def odom_callback(self, msg):
        ...
        self.pos_var = 0.0025 # msg.pose.covariance[0]
        ...
        self.path_length += np.linalg.norm(self.pos - self.old_pos)
        ...
        if self.initial_obs:
            ...
            # publish the velocity
            self.cmd_vel.publish(move_cmd)
        elif not self.replan and not self.goal_status and self.goal is not None:
            ...
            self.ang_error_array[self.error_index] = np.abs(angle_error)
            self.vel_error_array[self.error_index] = norm(to_waypoint)

            if np.sum(self.vel_error_array) > 0.8 and not self.replan:
                print('replann because hit obstacles')
                self.replan = True

            self.ang_error_log.append(np.sum(self.ang_error_array))
            self.vel_error_log.append(np.sum(self.vel_error_array))

            self.error_index += 1
            self.error_index = self.error_index / 100
            ...
            move_cmd = Twist()
            # let's go forward at 0.2 m/s
            move_cmd.linear.x = new_velocity
            # let's turn at 0 radians/s
            move_cmd.angular.z = self.angle_tracking(angle_error)

            self.vel_command = move_cmd.linear.x
            self.ang_command = move_cmd.angular.z
            ...
            if self.pos is not None and self.map_client.wait_for_result():
                ...
                for room_id in self.room_ids_2_objects:
                    ...
                    for obj_id in objects:
                        ...
                        evidence_indexs = np.nonzero(class_prob > 0.5)[0]

                        if evidence_indexs.size:
                            evidence.add(self.classes[evidence_indexs[0]])

                    req = self.c(list(evidence))
                ...
                for i in range(1, self.frontier_num_labels):
                    ...
                    if (room_id == 0 or room_id == 65280) and valid_pnts.size:
        ...
        if self.obj_of_interest is not None:
            ...
            for angle in angles:
                ...
                for pt in discrete_line:
                    ...
                    if self.grid_map_grey[x, y] == 0: #!=255
                        break
    ...
    def findFrontier(self):
        ...
        obstacle_grids = cv2.dilate(obstacle_grids, kernel, iterations=2,
        ...
        cv2.circle(frontier_mat, self.to_grid(self.pos),
                    int(self.blind_radius / self.resolution), 0, -1)
        ...
        #self.frontier_map[roi] = 0
        #cv2.imwrite('/home/zhentian/TurtleBot/roi.jpg', roi.astype(np.uint8))
        #cv2.imwrite('/home/zhentian/TurtleBot/frontier.jpg', (self.frontier_map * 255 / np.amax(self.frontier_map)
                     #).astype(np.uint8)) 


        for obj_id in self.sightings_map:
            self.sightings_map[obj_id][invalid_frontier] = 0 # obstacle_grids
